...
class BaseDao
{
    ...
    /**
     * transformenle objet en tableau
     */
    ...
    /**
     * Retourne l'utilisateur correspondant au pseudo en parametre
     */
    public function findByPseudo($pseudo)
    {
        $sql ="SELECT * FROM" . $this->getNomTable() . "WHERE pseudo = :psuedo";
 ...
 /**
  * Retourne l'utlisateur correspondant a l'offer en param
  */


    public function findByOfffre($offre){
        $sql = "SELECT * FROM" . $this->getNomTable() . "WHERE offre = :offre";
        try
        {
            $connexion = new Connexion ();
            $requete = $connexion->prepare($sql);
            $requete->execute(["offre" => $offre]);
            return $this->transformeTableauEnObjet($requete->fetch());
        }catch(PDOException $e)
        {
            echo "error...:(";
    ...
    /**
     * delete les id correspondante au parametre
     */
