...
if ($useAJAX) {
	...
	$includes.="var INLINE_EDIT_TABLE='globals_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='globals_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='globals_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='globals_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='globals_masterpreview.php';\r\n";
...
$smarty->assign("order_dir_TimePeriod","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$smarty->assign("order_dir_TimePeriod","a");
	if($order_field=="TimePeriod")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_TimePeriod","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_TimePeriod","<img src=\"images/".$img.".gif\" border=0>");
	}
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("TimePeriod", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='globals_list.php?goto='+nPageNumber;
		}
</script>");
...
//	TimePeriod - 
			$value="";
				$value = ProcessLargeText(GetData($data,"TimePeriod", ""),"field=TimePeriod".$keylink,"",MODE_LIST);
			$row[$col."TimePeriod_value"]=$value;
			$row[$col."show"]=true;
...
//	fill in search variables
	//	field selection
		if(@$_SESSION[$strTableName."_searchfield"]=="TimePeriod")
			$smarty->assign("search_TimePeriod","selected");
