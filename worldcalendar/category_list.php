...
if ($useAJAX) {
	...
	$includes.="var INLINE_EDIT_TABLE='category_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='category_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='category_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='category_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='category_masterpreview.php';\r\n";
...
$smarty->assign("order_dir_price","a");
$smarty->assign("order_dir_taskdetails","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	if($order_field=="price")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_price","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_price","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_taskdetails","a");
	if($order_field=="taskdetails")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_taskdetails","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_taskdetails","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_picture","a");
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	else
	{
		...
		if($where=StrWhere("Category", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("price", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("taskdetails", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='category_list.php?goto='+nPageNumber;
		}
</script>");
...
//	key fields
			...
			$row[$col."recno"] = $recno;
//	detail tables
			$masterquery="mastertable=category";
			$masterquery.="&masterkey1=".rawurlencode($data["id"]);
			$row[$col."task_statistics_masterkeys"]=$masterquery;
//	edit page link
			$editlink="";
...
//	fill in search variables
	//	field selection
		...
		if(@$_SESSION[$strTableName."_searchfield"]=="price")
			$smarty->assign("search_price","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="taskdetails")
			$smarty->assign("search_taskdetails","selected");
...
$templatefile = "category_list.htm";
