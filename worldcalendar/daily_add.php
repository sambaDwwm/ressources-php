...
{
	$defvalues["DateField"]=$_SESSION["yr"]."-".$_SESSION["mon"]."-".$_SESSION["days"];
	$defvalues["TimeField"]=$_SESSION["hour"];
	$defvalues["EndDate"]=($_SESSION["yr"]+1)."-".$_SESSION["mon"]."-".$_SESSION["days"];
	$defvalues["Period"]="d";
}
if($readavalues)
{
...
foreach($defvalues as $key=>$value)
	...
	$includes.="</script>\r\n";
			...
			$bodyonload.="define('value_DayEvent','".$validatetype."','Whole Day Event');";
				$validatetype="";
			if($validatetype)
			$bodyonload.="define('value_Recurrence','".$validatetype."','Recurrent Event');";
