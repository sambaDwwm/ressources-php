...
//	check if entered username already exists
	...
	{
		$strSQL="select count(*) from `users` where `username`=".add_db_quotes("username",$strUsername);
...
$smarty->assign("value_group",@$values["group"]);
$smarty->assign("value_info",@$values["info"]);
