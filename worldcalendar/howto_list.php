...
if ($useAJAX) {
	...
	$includes.="var INLINE_EDIT_TABLE='howto_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='howto_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='howto_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='howto_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='howto_masterpreview.php';\r\n";
...
$smarty->assign("order_dir_ask","a");
$smarty->assign("order_dir_answer","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	if($order_field=="ask")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_ask","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_ask","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_answer","a");
	if($order_field=="answer")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_answer","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_answer","<img src=\"images/".$img.".gif\" border=0>");
	}
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("ask", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("answer", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='howto_list.php?goto='+nPageNumber;
		}
</script>");
...
//	ask - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ask", ""),"field=ask".$keylink,"",MODE_LIST);
...
//	fill in search variables
	//	field selection
		if(@$_SESSION[$strTableName."_searchfield"]=="ask")
			$smarty->assign("search_ask","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="answer")
			$smarty->assign("search_answer","selected");
