...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	if($order_field=="idus")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_idus","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_idus","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_username","a");
	...
	if($order_field=="group")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_group","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_group","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_email","a");
	...
	if($order_field=="info")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_info","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_info","<img src=\"images/".$img.".gif\" border=0>");
	}
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["idus"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
//	key fields
			$keyblock="";
			$row[$col."id1"]=htmlspecialchars($data["idus"]);
			$keyblock.= rawurlencode($data["idus"]);
			$row[$col."keyblock"]=htmlspecialchars($keyblock);
//	detail tables
//	edit page link
			$editlink="";
			$editlink.="editid1=".htmlspecialchars(rawurlencode($data["idus"]));
			$row[$col."editlink"]=$editlink;

			$copylink="";
			$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["idus"]));
...
//	idus - 
			$value="";
				$value = ProcessLargeText(GetData($data,"idus", ""),"field=idus".$keylink,"",MODE_LIST);
			$row[$col."idus_value"]=$value;

//	group - 
			$value="";
				$value = ProcessLargeText(GetData($data,"group", ""),"field=group".$keylink,"",MODE_LIST);
			$row[$col."group_value"]=$value;
...
//	info - 
			$value="";
				$value = ProcessLargeText(GetData($data,"info", ""),"field=info".$keylink,"",MODE_LIST);
			$row[$col."info_value"]=$value;
			$row[$col."show"]=true;
