...
function GetTableURL($table="")
{
	...
	if("howto"==$table) 
		return "howto";
	if("editcalendar"==$table) 
		return "Copy_of_calendar";
	if("customer statistics"==$table) 
		return "customer_statistics";
	if("task statistics"==$table) 
...
// long binary data?
	...
	else if($format == FORMAT_FILE_IMAGE)
	{
		...
		$thumbnailed=false;
		$thumbprefix="";
		if($strTableName=="category" && $field=="picture")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="calendar")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="category")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="daily")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="monthly")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="weekly")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="globals")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="yearly")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="users")
		{
			$iquery.="&key1=".rawurlencode($data["idus"]);
		}
		if($strTableName=="customer")
		{
			$iquery.="&key1=".rawurlencode($data["idcustomer"]);
		}
		if($strTableName=="howto")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="editcalendar")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="customer statistics")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
		if($strTableName=="task statistics")
		{
			$iquery.="&key1=".rawurlencode($data["id"]);
		}
...
if ($data["Recurrence"]==1)
	...
	if($table=="editcalendar" && $field=="Theme") 
	{
		global $data;
if ($data["DateField2"])
	if ((strtotime(now())-strtotime($data["DateField2"]." ".$data["TimeField"]))>=0 && $data["DayEvent"]!=1 || (strtotime(date("Y-m-d"))-strtotime($data["DateField2"]))>0 && $data["DayEvent"]==1)	
...
if ($data["Recurrence"]==1)
	...
	if($table=="customer statistics" && $field=="Theme") 
	{
		global $data;
if ($data["DateField2"])
	if ((strtotime(now())-strtotime($data["DateField2"]." ".$data["TimeField"]))>=0 && $data["DayEvent"]!=1 || (strtotime(date("Y-m-d"))-strtotime($data["DateField2"]))>0 && $data["DayEvent"]==1)	
...
if ($data["Recurrence"]==1)
	...
	if($table=="task statistics" && $field=="Theme") 
	{
		global $data;
if ($data["DateField2"])
	if ((strtotime(now())-strtotime($data["DateField2"]." ".$data["TimeField"]))>=0 && $data["DayEvent"]!=1 || (strtotime(date("Y-m-d"))-strtotime($data["DateField2"]))>0 && $data["DayEvent"]==1)	
...
//	users
	if($table=="users")
	{
			$value=make_db_value("idus",$keys["idus"]);
		if($value==="null")
			$strWhere.=GetFullFieldName("idus")." is null";
		else
			$strWhere.=GetFullFieldName("idus")."=".make_db_value("idus",$keys["idus"]);
	}

//	customer
	if($table=="customer")
	{
			$value=make_db_value("idcustomer",$keys["idcustomer"]);
		if($value==="null")
			$strWhere.=GetFullFieldName("idcustomer")." is null";
		else
			$strWhere.=GetFullFieldName("idcustomer")."=".make_db_value("idcustomer",$keys["idcustomer"]);
	}
...
function GetUserPermissions($table="")
{
	...
	if($table=="calendar" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="calendar" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="calendar")
		return "AEDSP";
	if($table=="category" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="category" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="category")
		return "AEDSP";
	if($table=="daily" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="daily" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="daily")
		return "AEDSP";
	if($table=="monthly" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="monthly" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="monthly")
		return "AEDSP";
	if($table=="weekly" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="weekly" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="weekly")
		return "AEDSP";
	if($table=="globals" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="globals" && $sUserGroup=="<Guest>")
				return "S";
//	default permissions	
	if($table=="globals")
		return "AEDSP";
	if($table=="yearly" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="yearly" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="yearly")
		return "AEDSP";
...
//	default permissions	
	...
	if($table=="customer" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="customer" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="customer")
		return "AEDS";
	if($table=="howto" && $sUserGroup=="admin")
				return "AEDSP";
	if($table=="howto" && $sUserGroup=="<Guest>")
				return "S";
//	default permissions	
	if($table=="howto")
		return "AEDS";
	if($table=="editcalendar" && $sUserGroup=="admin")
				return "ADESP";
	if($table=="editcalendar" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="editcalendar")
		return "ADES";
	if($table=="customer statistics" && $sUserGroup=="admin")
				return "ADESP";
	if($table=="customer statistics" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="customer statistics")
		return "ADES";
	if($table=="task statistics" && $sUserGroup=="admin")
				return "ADESP";
	if($table=="task statistics" && $sUserGroup=="<Guest>")
				return "";			//	guest sees nothing
//	default permissions	
	if($table=="task statistics")
		return "ADES";
...
function BuildSelectControl($field, $value, $values="", $secondfield=false, $mode, $id="")
{
	...
	$script="";
		if($strTableName=="calendar" && $field=="Description") 
		{
			$addnewitem= false ;
			$LinkField="idcustomer";
			$DisplayField="name";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.="where "." idusercus = ".$_SESSION["OwnerID"];
			
						$LookupSQL.=" ORDER BY `customer`.`name`";
			
		}
		if($strTableName=="calendar" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.="where "." idusercat = ".$_SESSION["OwnerID"];
			
						$LookupSQL.=" ORDER BY `category`.`id`";
			
		}
		if($strTableName=="daily" && $field=="Description") 
		{
			$addnewitem= false ;
			$LinkField="idcustomer";
			$DisplayField="name";
		...
		if($strTableName=="daily" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			$DisplayField="Category";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `category`.`Category`";
			
		}
		if($strTableName=="daily" && $field=="Period") 
		{
			$addnewitem= false ;
			$LinkField="ValueField";
			$DisplayField="Period";
			$LookupTable="period";
			$strSize=1;
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `period`.`id`";
			
		}
		if($strTableName=="globals" && $field=="TimePeriod") 
		{
			$addnewitem= false ;
			$LinkField="";
			$DisplayField="";
		...
		if($strTableName=="yearly" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			$DisplayField="Category";
		...
		if($strTableName=="editcalendar" && $field=="Description") 
		{
			$addnewitem= false ;
			$LinkField="idcustomer";
			$DisplayField="name";
		...
		if($strTableName=="editcalendar" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			$DisplayField="Category";
		...
		if($strTableName=="customer statistics" && $field=="Description") 
		{
			$addnewitem= false ;
			$LinkField="idcustomer";
			$DisplayField="name";
		...
		if($strTableName=="customer statistics" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			$DisplayField="Category";
		...
		if($strTableName=="task statistics" && $field=="Description") 
		{
			$addnewitem= false ;
			$LinkField="idcustomer";
			$DisplayField="name";
		...
		if($strTableName=="task statistics" && $field=="Category") 
		{
			$addnewitem= false ;
			$LinkField="id";
			$DisplayField="Category";
...
//	get the initial value
			...
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`idcustomer`";
								$lookup_SQL .= ",`name`";
				$lookup_SQL .= " FROM `customer` ";
								$lookup_SQL.="where ("." idusercus = ".$_SESSION["OwnerID"].")  AND `idcustomer`=".make_db_value($field,$value)."";
			...
			if($strTableName=="calendar" && $field=="Category") 
			{
								...
								$lookup_SQL .= "`id`";
								$lookup_SQL .= ",`Category`";
				$lookup_SQL .= " FROM `category` ";
								$lookup_SQL.="where ("." idusercat = ".$_SESSION["OwnerID"].")  AND `id`=".make_db_value($field,$value)."";
			...
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`ValueField`";
								$lookup_SQL .= ",`Period`";
				$lookup_SQL .= " FROM `period` ";
								$lookup_SQL .= " WHERE `ValueField`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `id`";
								}
			if($strTableName=="globals" && $field=="TimePeriod") 
			{
			...
			if($strTableName=="yearly" && $field=="Category") 
			{
								...
								$lookup_SQL .= ",`Category`";
				$lookup_SQL .= " FROM `category` ";
								$lookup_SQL .= " WHERE `id`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `id`";
								}
			if($strTableName=="editcalendar" && $field=="Description") 
			{
			...
			else
			{
				if($strTableName=="calendar" && $field=="Description") 
				{
										...
										$lookup_SQL .= ",`name`";
					$lookup_SQL .= " FROM `customer` ";
					$lookup_SQL .= " WHERE `idcustomer`=".make_db_value($field,$value)."";
											$lookup_SQL.= " ORDER BY `name`";
										}
				if($strTableName=="calendar" && $field=="Category") 
				{
...
//	build the control
		...
		else
		{
			...
			if(	$strTableName=="calendar" && $field=="Description"
				&&$mode==MODE_EDIT && $mode==MODE_INLINE_EDIT && !$found && strlen($value)) 
			{
				$LookupSQL = "select ";
						$LookupSQL .= "`idcustomer`";
								$LookupSQL .= ",`name`";
				...
				$LookupSQL.=" where `idcustomer`=".make_db_value($field,$value);
				...
				&&$mode==MODE_EDIT && $mode==MODE_INLINE_EDIT && !$found && strlen($value)) 
			{
				...
				$LookupSQL.=" where `id`=".make_db_value($field,$value);
...
function BuildEditControl($field , $value, $format, $edit, $secondfield=false, $id="")
{
	...
	if($strTableName=="users")
	{
		$keylink.="&key1=".rawurlencode($keys["idus"]);
		$iquery.=$keylink;
	}
	if($strTableName=="customer")
	{
		$keylink.="&key1=".rawurlencode($keys["idcustomer"]);
		$iquery.=$keylink;
	}
	if($strTableName=="howto")
	{
