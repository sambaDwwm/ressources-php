 ...
 *	Class Insurances 
 *  --------------
 *	Description : encapsulates insurances methods and properties
 *	Written by  : ApPHP
 *	Version     : 1.0.0
 *  Updated	    : 16.09.2012
...
class Insurances extends MicroGrid {
	...
	{		
		...
		$this->primaryKey 	= 'id';
		$this->tableName 	= TABLE_INSURANCES; // 
		$this->dataSet 		= array();
		$this->error 		= '';
		$this->formActionURL = 'index.php?admin=mod_appointments_insurances';
		...
		$this->VIEW_MODE_SQL = 'SELECT
									...
									id.name as insurance_name
								FROM ('.$this->tableName.' 
									LEFT OUTER JOIN '.TABLE_INSURANCES_DESCRIPTION.' id ON '.$this->tableName.'.id = id.insurance_id AND id.language_id = \''.$this->languageId.'\')
								';		
								
		// define view mode fields
		$this->arrViewModeFields = array(
			'insurance_name' => array('title'=>_NAME, 'type'=>'label', 'align'=>'left', 'width'=>'', 'sortable'=>true, 'nowrap'=>'', 'visible'=>true, 'tooltip'=>'', 'maxlength'=>'', 'format'=>'', 'format_parameter'=>''),
	...
	{		
		$sql = 'SELECT
					...
					id.name,
					id.description
				FROM '.TABLE_INSURANCES.' i
					INNER JOIN '.TABLE_INSURANCES_DESCRIPTION.' id ON i.id = id.insurance_id AND id.language_id = \''.Application::Get('lang').'\'
				WHERE
					i.is_active = 1
	...
	public function AfterInsertRecord()
	{
		...
		$sql = 'INSERT INTO '.TABLE_INSURANCES_DESCRIPTION.'(id, insurance_id, language_id, name, description) VALUES ';
		$count = 0;
		foreach($this->arrTranslations as $key => $val){
			if($count > 0) $sql .= ',';
	...
	public function AfterUpdateRecord()
	{
		...
		foreach($this->arrTranslations as $key => $val){
			$sql = 'UPDATE '.TABLE_INSURANCES_DESCRIPTION.'
	...
	public function AfterDeleteRecord()
	{
		...
		$sql = 'DELETE FROM '.TABLE_INSURANCES_DESCRIPTION.' WHERE insurance_id = '.$this->curRecordId;
		if(database_void_query($sql)){
			return true;
		}else{
			return false;
