 ...
 *      - mysql _real_escape_string() replaced with encode_text()
...
class Pages {
	...
	public function DrawText($draw = true)
	{
		...
		if(!$objLogin->IsLoggedInAsAdmin() && !empty($this->page['finish_publishing']) && date('Y-m-d') > $this->page['finish_publishing']){
	...
	public function CacheAllowed()
	{
        ...
        if(self::$PROJECT == 'HotelSite' && preg_match('/{module:rooms}/i', $this->page['page_text'])){
            return false;
        }
		return true;
	}	
