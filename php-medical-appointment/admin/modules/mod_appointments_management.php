...
if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('appointments')){	
	...
	if($action=='add'){		
		...
		if($objAppointments->DeleteRecord($rid)){
	...
	// Start main content
	draw_title_bar(
		prepare_breadcrumbs(array(_APPOINTMENTS=>'',_MANAGEMENT=>'',_APPOINTMENTS_MANAGEMENT=>'',ucfirst($action)=>'')),
