...
if($objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	if($tabid == '1_1') {
                ...
                <td style="border-left:1px solid #d1d2d3;padding:2px 18px 2px 18px">
                    ...
                    foreach($social_networks as $key => $val){
                        $tab_content_6 .= '<tr valign="middle">
                            ...
                            <td>
                                '.(($val['url'] != '') ? '<a href="'.$val['url'].'" target="_blank" rel="noopener noreferrer">' : '').'
		...
		$tab_content_7 .= '<ul '.((Application::Get('lang_dir') == 'rtl') ? 'style="list-style:none"' : '').'>
				...
				<li><b>'.((DB_TYPE == 'PDO') ? 'PDO MySQL' : DB_TYPE).' Version:</b> <i>'.(isset($mysqli_version['ve']) ? $mysqli_version['ve'] : _UNKNOWN).'</i></li>
